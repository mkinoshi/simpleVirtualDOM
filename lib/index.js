...
const updateIfNecessary = (element, oldElement, newElement, ind) => {
  if (!oldElement && !newElement) {
    ...
    const maxInd = newElement.children.length > oldElement.children.length ? newElement.children.length : oldElement.children.length;
...
const updateChildrenWithKeys = (element, oldElementChildren, newElementChildren) => {
  ...
  const { deletedKeys, insertedKeys, staticKeys, unsortedStatickeys } = categorizeKeys(uniqueKeys, keysInOldObject, keysInNewObject, keysInOldElement);
  let count = 0; // this variable keeps track of the items added
  // First handle reorder of keys that stay the same
  handleReorder(element, staticKeys, unsortedStatickeys, keysInOldObject, keysInNewObject, keysInOldElement, oldElementChildren, newElementChildren);
...
const handleReorder = (element, staticKeys, unsortedStatickeys, keysInOldObject, keysInNewObject, keysInOldElement, oldElementChildren, newElementChildren) => {
  const replacedKeys = [];
  let flipped = false;
  for (let i = 0; i < staticKeys.length; i++) {
    const oldInd = unsortedStatickeys.indexOf(staticKeys[i]);
    const newInd = i;
    if (oldInd !== newInd && (replacedKeys.indexOf(keysInOldElement[oldInd]) === -1 || replacedKeys.indexOf(keysInOldElement[newInd]) === -1)) {
      ...
      let secondRealInd = keysInOldObject[unsortedStatickeys[i]];
      let firstInd = oldInd;
      ...
      replacedKeys.push(unsortedStatickeys[firstInd]);
      replacedKeys.push(unsortedStatickeys[secondInd]);
      ...
      // Here I update both unsortedStatickeys and DOM 
      const firstTmp = element.removeChild(element.childNodes[secondRealInd]);
      const valTmp = unsortedStatickeys.splice(secondInd, 1)[0];
      element.insertBefore(firstTmp, element.childNodes[firstRealInd]);
      unsortedStatickeys.splice(firstInd, 0, valTmp);
      ...
      if (secondInd - firstInd > 1) {
        const secondValTmp = unsortedStatickeys.splice(firstInd + 1, 1)[0];
        unsortedStatickeys.splice(secondInd, 0, secondValTmp);
      }

      // You also have to make sure to update the key-index object
      const indTmp = keysInOldObject[unsortedStatickeys[secondInd]];
      keysInOldObject[unsortedStatickeys[secondInd]] = keysInOldObject[unsortedStatickeys[firstInd]];
      keysInOldObject[unsortedStatickeys[firstInd]] = indTmp;
      flipped = true;
    }
  }
  for (let j = 0; j < staticKeys.length; j++) {
    ...
    const maxInd = newElementChildren[newInd].children.length > oldElementChildren[oldInd].children.length ? newElementChildren[newInd].children.length : oldElementChildren[oldInd].children.length;
    for (let i = 0; i < maxInd; i++) {
...
const categorizeKeys = (uniqueKeys, keysInOldObject, keysInNewObject) => {
  ...
  const unsortedStatickeys = staticKeys.slice(0);
  staticKeys.sort((a, b) => keysInNewObject[a] - keysInNewObject[b]);
  return { deletedKeys, insertedKeys, staticKeys, unsortedStatickeys };
};
...
const needChange = (oldElement, newElement) => {
  ...
  return oldElement.type !== newElement.type || typeof oldElement !== typeof newElement || needChangeProps(oldElement.props, newElement.props);
};
